...
class TimeSeriesDifficultyWeight:
    ...
    def get_mixed_sampling_probabilities(self, alpha=0.7, min_prob=0.01):
        """Mix between hard and easy sampling."""
...
for model_name in MODEL_NAMES:
    ...
    for rnd in range(NUM_ROUNDS):
        ...
        for cluster_name, client_ids_cluster in CLUSTERS.items():
            if len(client_ids_cluster) == 0:
                continue

            # Convert real IDs -> internal indices
            cluster_indices = np.array([id2idx[int(cid)] for cid in client_ids_cluster])
        ...
        print(f"Sampled {len(sampled_clients)} clients across all clusters")

        # --- Local SCAFFOLD training on sampled clients ---
        local_weight_deltas = []
        local_c_deltas = []
        ...
        checkpoint_path = os.path.join(
            checkpoint_dir,
            f"{model_name}_round_{rnd+1}_AEpublic_k-means_SCAFFOLD.pt"
        )
