...
# Updated SimpleAE class with 1D-CNN Encoder
class SimpleAE(nn.Module):
    def __init__(self, input_dim, latent_dim):
        super().__init__()
        # Input dim is the sequence length (e.g., 168)
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, stride=2, padding=1),  # Downsample
            nn.ReLU(),
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=3, stride=2, padding=1),  # Downsample
            nn.ReLU(),
            nn.Flatten(),
            # Compute the flattened size: after two stride-2 convolutions, length is roughly input_dim/4
            nn.Linear(64 * (input_dim // 4), latent_dim),
            nn.ReLU()
        )

        # Decoder: Feedforward network to reconstruct input
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 64 * (input_dim // 4)),
            nn.ReLU(),
            nn.Linear(64 * (input_dim // 4), input_dim),
            # No activation at the output to allow for real-valued reconstruction
        )

    def forward(self, x):
        # Reshape input for 1D-CNN: (batch_size, input_dim) -> (batch_size, 1, input_dim)
...
# For covariates, keep the original SimpleAE since it's not a time series
...
with torch.no_grad():
    Z_tr = ae_trend.encoder(torch.tensor(trend_real, dtype=torch.float32).to(DEVICE).unsqueeze(1)).cpu().numpy()
    Z_se = ae_season.encoder(torch.tensor(seasonal_real, dtype=torch.float32).to(DEVICE).unsqueeze(1)).cpu().numpy()
    Z_re = ae_resid.encoder(torch.tensor(resid_real, dtype=torch.float32).to(DEVICE).unsqueeze(1)).cpu().numpy()
...
# TimeSeriesDifficultyWeight class (unchanged)
...
for model_name in MODEL_NAMES:
    ...
    for rnd in range(NUM_ROUNDS):
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f"{model_name}_round_{rnd+1}_CNN-clustering.pt")
