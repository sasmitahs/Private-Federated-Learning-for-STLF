...
Model: dual_cnn_gru_fcnn, Strategy: SCAFFOLD_nocluster_correct
[DEBUG] rolling_forecast_on_test: CID=1408
[INFO] Saved predictions to /home/user/DPFL-Sasmita/FL-Baseline-Codes/predictions40-50-168-T/1408_dual_cnn_gru_fcnn_SCAFFOLD_nocluster_correct.csv
Metrics saved to /home/user/DPFL-Sasmita/FL-Baseline-Codes/metrics40-50-168-T/cid1408_dual_cnn_gru_fcnn_SCAFFOLD_nocluster_correct_metrics.csv

Results for Strategy: SCAFFOLD_nocluster_correct
Number of processed metric files: 1307
Median MAE: 11.1513
Median MSE: 201.2126
Median RMSE: 14.1849
Median MAPE (%): 27.1624
Median SMAPE (%): 19.7682
Median NRMSE: 0.1414


Centralised on each model

dual_cnn_gru_fcnn_round_1_AEpublic_k-means_2enc.pt
dual_cnn_gru_fcnn_round_1_no-cluster_no-AE.pt
dual_cnn_gru_fcnn_round_1_poc.pt
dual_cnn_gru_fcnn_round_1_poc_nocluster.pt
...
def plot_convergence_graphs(
    ...
    """
    Plot convergence graphs for each metric and a time series plot for the median client.
    """
    ...
    for metric in metrics:
        ...
        for strat_name in strategies:
            ...
            plt.plot(rounds, values, label=strat_name, marker='o')
        
        plt.xlabel('Round')
        plt.ylabel(metric)
        plt.title(f'Convergence of {metric} for Client {median_client}')
        plt.legend()
        plt.grid(True)
        plot_path = os.path.join(save_dir, f"convergence_{metric.lower().replace(' ', '_').replace(' (%)', 'pct')}_client_{median_client}.png")
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        logging.info(f"Saved convergence plot: {plot_path}")
    
    # Time series plot (true vs pred for the last valid round)
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle(f'Time Series Predictions for Client {median_client}', fontsize=16)
    axes = axes.flatten()
    
    for idx, (strat_name, aggr_strat) in enumerate(strategies.items()):
        if idx >= 4: break
        ax = axes[idx]
        
        # Find the last round with valid predictions
        last_valid_round = max([r for r in rounds if os.path.exists(os.path.join(model_dir, f"{model_name}_round_{r}_{aggr_strat}.pt"))], default=None)
        if last_valid_round is None:
            logging.info(f"[WARN] No valid model for {strat_name}")
            continue
        
        df, _ = get_predictions_and_metrics(
            model_name=model_name,
            cid=median_client,
            round_num=last_valid_round,
            model_dir=model_dir,
            aggr_strat=aggr_strat,
            filepath=filepath
        )
        ...
        if not df.empty:
            # Sample first 500 points for clarity
            sample_df = df.head(500)
            ax.plot(pd.to_datetime(sample_df['timestamp']), sample_df['true'], label='True', alpha=0.7)
            ax.plot(pd.to_datetime(sample_df['timestamp']), sample_df['pred'], label='Pred', alpha=0.7)
            ax.set_title(f'{strat_name} (Round {last_valid_round})')
            ax.legend()
            ax.tick_params(axis='x', rotation=45)
    
    plt.tight_layout()
    ts_plot_path = os.path.join(save_dir, f"time_series_convergence_client_{median_client}.png")
