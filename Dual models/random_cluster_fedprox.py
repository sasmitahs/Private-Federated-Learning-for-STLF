...
clusters = {k: [client_ids[i] for i in range(len(client_ids)) if labels[i]==k] for k in range(K)}
for k, ids in clusters.items():
    print(f"cluster_{k}: {len(ids)} buildings; sample: {ids[:5]}")
...
# -----------------------------
# Global training loop with RANDOM SAMPLING and FedProx
# -----------------------------
for model_name in MODEL_NAMES:
    ...
    for rnd in range(NUM_ROUNDS):
        ...
        for cluster_id, client_ids in CLUSTERS.items():
            ...
            n_sample = max(1, int(CLIENT_FRAC * len(client_ids)))
            
            # RANDOM sampling (uniform probability)
            sampled_from_cluster = random.sample(list(client_ids), n_sample)
        ...
        # --- Local training on sampled clients with FedProx ---
        local_weights = []
        ...
        # --- Standard aggregation (uniform weights, as in FedAvg) ---
        if local_weights:
