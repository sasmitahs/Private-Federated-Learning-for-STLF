...
class RawEncoder(nn.Module):
    ...
    def forward(self, x, time_emb):
        # Combine input with time embedding if provided
...
class Decoder(nn.Module):
    def __init__(self, in_dim, out_horizon, hidden_dim=64):
        super().__init__()
        self.gru = nn.GRU(
            input_size=in_dim,
            hidden_size=hidden_dim,
    ...
    def forward(self, h, ts_future=None):
        # h is (batch, features) from encoder
        # Reshape h to (batch, 1, features) for GRU
        h = h.unsqueeze(1)
        # GRU expects input of shape (batch, seq_len, input_size)
        out, _ = self.gru(h)
        # Take the last time step: (batch, 1, hidden_dim) -> (batch, hidden_dim)
        out = out[:, -1, :]
        # Map to output horizon
        out = self.fc(out)
        return out
...
class DualStreamModel(nn.Module):
    ...
    def forward(self, x_raw, x_decomp, covariates, ts_window, ts_future):
        ...
        h_raw = h_raw.mean(dim=1)  # Aggregate over sequence
        if self.use_covariates and self.cov_encoder is not None:
            h_cov = self.cov_encoder(covariates)
            h_cov = h_cov.mean(dim=1)  # Aggregate over sequence
            h = torch.cat([h_raw, h_cov], dim=-1)
        else:
            h = h_raw
        return self.decoder(h, ts_future)
...
if __name__ == "__main__":
    ...
    run_ablation(df, ablation_configs, out_csv="ablation_results_central_gru-dec+cnn.csv")